$(document).ready(function()
{
		...
		ipllivescore();
		$("#ipl_video_thumbnail1").YouTubePopup({ youtubeId: 'AMif3k0NKNI', title: 'Bindaas Cricket Café !' });
		$("#ipl_video_thumbnail2").YouTubePopup({ youtubeId: 'AMif3k0NKNI', title: 'Bindaas Cricket Café !' });
		getpoll();
		...
		success:function(data)
		{
			...
			if(data.resultset.success===2)
			{
				...
				$.each(data.resultset.vote_rate, function(i, item) {
					...
					$("#pollans"+i+" > div").css({ 'background': '#784a9f' });
					$("#pollans"+i).css({'border':'1px solid #bbbbbb','background': '#cccccc','border-radius':'0px'});
				});
			}
		},
		error:function()
