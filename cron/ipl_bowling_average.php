...
class Ipl_bowling_avgs extends ActiveRecord\Model { }
...
try
	{
		...
		$json_output=array();
		$data_list=array();
		$jsonurl = "http://dynamic.pulselive.com/dynamic/data/core/cricket/2012/ipl2013/stats/player/full/bestBowlingAverage.js?_".time();
		...
		foreach ($json_output['bestAverage'][0]['topPlayers'] as $single) {
		...
		if(count($data_list)>0)
		{
			...
			Ipl_bowling_avgs::connection()->query('TRUNCATE TABLE `ipl_bowling_avgs`');
			foreach($data_list as $row)
			{
						Ipl_bowling_avgs::create($row);
			}
		}
	}
	catch (Exception $e)
