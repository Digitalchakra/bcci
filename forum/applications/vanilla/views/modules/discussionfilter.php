...
//      if (C('Vanilla.Categories.ShowTabs')) {
         ...
         echo '<li class="nav-menu">'.Anchor(Sprite('SpAllCategories').' '.T('Categories'), '/categories').'</li> ';
//      }
      ?>
      <li class="nav-menu<?php echo strtolower($Controller->ControllerName) == 'discussionscontroller' && strtolower($Controller->RequestMethod) == 'index' ? ' Active' : ''; ?>"><?php echo Gdn_Theme::Link('forumroot', Sprite('SpDiscussions').' '.T('Recent Discussions')); ?></li>
      <?php echo Gdn_Theme::Link('activity', Sprite('SpActivity').' '.T('Activity'), '<li class="Activities nav-menu"><a href="%url" class="%class">%text</a></li>'); ?>
      <?php if ($CountBookmarks > 0 || $Controller->RequestMethod == 'bookmarked') { ?>
      <li class="nav-menu<?php echo $Controller->RequestMethod == 'bookmarked' ? ' Active' : ''; ?>"><?php echo Anchor(Sprite('SpBookmarks').' '.$Bookmarked, '/discussions/bookmarked'); ?></li>
      <?php
      }
      if (($CountDiscussions > 0 || $Controller->RequestMethod == 'mine') && C('Vanilla.Discussions.ShowMineTab', TRUE)) {
      ?>
      <li class="nav-menu<?php echo $Controller->RequestMethod == 'mine' ? ' Active' : ''; ?>"><?php echo Anchor(Sprite('SpMyDiscussions').' '.$MyDiscussions, '/discussions/mine'); ?></li>
      <?php
      }
      if ($CountDrafts > 0 || $Controller->ControllerName == 'draftscontroller') {
      ?>
      <li class="nav-menu<?php echo $Controller->ControllerName == 'draftscontroller' ? ' Active' : ''; ?>"><?php echo Anchor(Sprite('SpMyDrafts').' '.$MyDrafts, '/drafts'); ?></li>
      <?php
      }
